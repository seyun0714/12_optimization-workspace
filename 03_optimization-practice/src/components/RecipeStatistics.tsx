...
export default function RecipeStatistics({ recipe }: RecipeStatisticsProps) {
  const statistics = useMemo(() => {
    ...
    const ingredientCategories = {
      ...
      육류: recipe.ingredients.filter(
        (ing) =>
          ing.includes('소고기') ||
          ing.includes('돼지고기') ||
          ing.includes('닭고기')
      ).length,
      조미료: recipe.ingredients.filter(
        (ing) =>
          ing.includes('소금') ||
          ing.includes('후추') ||
          ing.includes('간장') ||
    ...
    <div className="mt-8 p-6 bg-gradient-to-br from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-lg">
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
        ...
        <div className="space-y-2">
          {Object.entries(statistics.ingredientCategories).map(
            ([category, count]) =>
              count > 0 && (
                <div
                  key={category}
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-32 h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${
                            (count / statistics.totalIngredients) * 100
                          }%`,
                        }}
                      />
                    </div>
